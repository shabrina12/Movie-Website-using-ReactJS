...
function Horror() {
  const [hororMovies, setHororMovieList] = useState([]);

  const fetchHororMovies = async() => { 
    try {
      ...
      if(data.Response === 'False') {
        console.log('Failed to fetch horor movies');
        setHororMovieList([]);
        return;
      }

      setHororMovieList(data.results || []);
      console.log("horor movie list: ", data.results);  
    } catch (error) {
  ...
  return (
    <div className='pt-16'>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <h2 className="text-8xl">Horor</h2>
